...
The argument unfolds in two movements: first, why large language models alone are
insufficient for multi-document legal case analysis; and second, why retrieval-augmented
generation --- the standard remedy --- carries fundamental limitations that make it
inadequate for cross-document reasoning. These limitations motivate the structured
extraction approach evaluated in this thesis. Section~\ref{sec:related-work} then
surveys prior work and alternative architectures that contextualise the design choices.
...
reside in a few isolated chunks, but is distributed across multiple documents.
...
an entire case file. The apparent alternative --- injecting the full document corpus into
context --- was ruled out in Section~\ref{sec:llm-context}: effective utilisation degrades
well before the nominal context limit is reached, cost and latency scale prohibitively with
document volume, and the design principle established there requires minimising rather than
maximising the information loaded at query time.
...
The final theoretical component concerns how structured knowledge is maintained across
sessions in a stateless architecture. As noted in the introduction, LLM inference
endpoints carry no state between calls: context must be reconstructed at every query.
Stateful agent frameworks such as LangGraph address this through checkpointing ---
persisting structured agent state to a database and reinjecting it at query time
\cite{langgraph}. For the conversational history specifically, rolling summarization
--- periodically condensing prior turns into a compact summary --- is a practical
strategy for preventing context overflow \cite{microsoft2024semantickernelpython}.
It is worth noting that rolling summarization applies to the chat inference pipeline
only; the document ingestion pipeline operates as a stateless batch process and does
not maintain rolling conversational context.
...
RAG has emerged as the standard architecture for domain-specific document QA, and its application to legal settings has been explored in several directions. 
% Louis et al.\ \cite{louis2023interpretablelongformlegalquestion} demonstrate that RAG-augmented LLMs achieve competitive performance on long-form 
% legal question answering while producing interpretable, citation-grounded responses. Their system retrieves relevant statutory passages and case law and incorporates 
% them as context at inference time --- the same basic mechanism used by the baseline agent in this thesis. Their work highlights both the strengths of RAG (traceable sourcing, factual grounding) 
% and its limitations when queries require synthesising evidence from across a large corpus.

Louis et al.\ \cite{louis2023interpretablelongformlegalquestion} demonstrate that a RAG-augmented LLM pipeline can generate long-form answers to statutory 
law questions with promising automatic metric scores, while incorporating extractive rationales for improved interpretability and citation grounding. 
Their system retrieves relevant statutory passages (but not case law) and incorporates them as context at inference time 
--- the same basic mechanism used by the baseline agent in this thesis. 
Their work highlights both the strengths of RAG (traceable sourcing, factual grounding) and its limitations when queries require 
synthesising evidence from across a large corpus or when hallucinations persist despite relevant retrieval.
...
Extracting structured information from unstructured legal documents --- parties, events, claims, and temporal relationships --- has been studied as an information extraction problem long before the LLM era. Recent work has revisited this problem with LLMs: Jiang et al.\ \cite{Jiang_2025} propose Retrieval and Structuring Augmented Generation (RSAG), which combines retrieval with LLM-driven structuring of retrieved content before generation. Their results show that structuring retrieved passages into typed entities before injecting them into the generation context consistently outperforms naive RAG on complex reasoning tasks. This provides independent empirical support for the core design hypothesis of this thesis: that structured, relational case state is more useful to an LLM than raw retrieved chunks.

GraphRAG \cite{edge2025localglobalgraphrag} extends this idea to the full document corpus, building a knowledge graph over entities and their relationships to enable community-level summarization and structured traversal. HopRAG \cite{liu2025hoprag} adds explicit multi-hop reasoning, constructing logical chains across documents rather than relying on chunk similarity alone. The factsheet implemented in this thesis can be understood as a lightweight, domain-specific instance of the same principle: rather than constructing a general-purpose property graph, it organises only the entities most relevant to legal case management into a typed, relational structure. The reduced generality is an acknowledged trade-off --- the system covers only what a legal practitioner is likely to ask rather than supporting arbitrary queries over the full document space. The practical advantages of this choice, in terms of query-time cost, schema precision, and interpretability, are examined in the evaluation and discussion.
...
Several of these mechanisms inform the design of the system in this thesis. The factsheet functions as a domain-specific persistent memory: it is initialised during document ingestion and injected directly into context at query time, bypassing the need for vector retrieval of case-level facts. In the chat inference pipeline, rolling summarisation of conversation history serves an analogous eviction function, preventing accumulated dialogue from consuming the context budget; the document ingestion pipeline, being a stateless batch process, does not require this mechanism.
