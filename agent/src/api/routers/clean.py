...
async def cleanup_all_metadata_endpoint(query: AskAgentRequest, 
    ...
    async def gen():
        try:
            async for chunk in clean_meta.astream({"query" : query}, config=thread, stream_mode="custom"):
...
@router.post("/clean-project-elements")
async def cleanup_project_elements_endpoint(query: CleanupElementsRequest,
    ...
    async def gen():
        try:
            async for chunk in clean_element.astream({"query" : query}, config=thread, stream_mode="custom"):
                yield f'data: {json.dumps(chunk)}\n\n'
                await asyncio.sleep(0.01)
        except Exception as e:
