...
import operator
from pydantic import BaseModel, Field
from models import EmailModel, AttachmentModel, Event, Damage, Claim, Deadline, Email, Attachment
...
class PipelineState(BaseModel):
    ...
    # Intermediate
    email_models: list[EmailModel] = Field(default_factory=list)
    ...
    damages:     Annotated[list[Damage], operator.add] = Field(default_factory=list)
    claims:      Annotated[list[Claim], operator.add] = Field(default_factory=list)
    deadlines:   Annotated[list[Deadline], operator.add] = Field(default_factory=list)
    attachments: Annotated[list[Attachment], operator.add] = Field(default_factory=list)
    emails:      Annotated[list[Email], operator.add] = Field(default_factory=list)
