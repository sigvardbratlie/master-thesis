...
def test_needs_ocr_with_text(doc_processor):
    ...
    with patch('database.vectorstore_modules.PdfReader') as mock_reader_cls:
        ...
        result = doc_processor.needs_ocr(b"fake pdf bytes")
        assert result is False
    ...
    with patch('database.vectorstore_modules.ocrmypdf') as mock_ocr:
        ...
        with patch('builtins.open', MagicMock()) as mock_open:
            ...
            with patch('os.unlink'):
                result = doc_processor.ocr_bytes(input_bytes)
...
def test_parse_pdf_with_text(doc_processor):
    ...
    with patch.object(doc_processor, 'needs_ocr', return_value=False):
        with patch('database.vectorstore_modules.PdfReader') as mock_reader_cls:
            mock_page = MagicMock()
...
def test_parse_pdf_triggers_ocr_when_needed(doc_processor):
    ...
    with patch.object(doc_processor, 'needs_ocr', return_value=True):
        with patch.object(doc_processor, 'ocr_bytes', return_value=b"ocr output") as mock_ocr:
            with patch('database.vectorstore_modules.PdfReader') as mock_reader_cls:
                mock_page = MagicMock()
...
def test_parse_text_short(doc_processor):
    ...
    result = doc_processor.parse_text(text, metadata)
...
def test_parse_text_long_produces_multiple_chunks(doc_processor_small_chunks):
    ...
    result = doc_processor_small_chunks.parse_text(text, metadata)
...
def test_parse_text_empty(doc_processor):
    ...
    metadata = get_mock_metadata()
    result = doc_processor.parse_text("", metadata)
    assert result == []
...
def test_process_attachment_pdf(doc_processor):
    """Test process_attachment routes PDF to parse_pdf with base64 decoding."""
    ...
    with patch.object(doc_processor, 'parse_pdf', return_value=[Document(page_content="parsed")]) as mock_parse:
        result = doc_processor.process_attachment(
            content=content,
            file_id=file_id,
        ...
        # First arg should be decoded bytes
        assert isinstance(call_args[0][0], bytes)
        # Metadata should have file_id and session_id
        assert call_args[1]["metadata"]["file_id"] == file_id
        assert call_args[1]["metadata"]["session_id"] == session_id


def test_process_attachment_text(doc_processor):
    """Test process_attachment routes text to parse_text."""
...
def test_process_attachment_invalid_base64(doc_processor):
    """Test process_attachment handles invalid base64 for PDF."""
    result = doc_processor.process_attachment(
